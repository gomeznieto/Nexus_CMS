...
namespace Backend_portafolio.Sevices
{
    ...
    public class RepositorySource : IRepositorySource
    {
        ...
        public async Task<IEnumerable<Source>> Obtener()
        {
            ...
            return await connection.QueryAsync<Source>($@"SELECT {SOURCE.ID}, {SOURCE.NAME}, {SOURCE.ICON} FROM {SOURCE.TABLA}");
        }
		...
		{
			using var connection = new SqlConnection(_connectionString);
			return await connection.QuerySingleAsync<Source>($@"SELECT {SOURCE.ID}, {SOURCE.NAME}, {SOURCE.ICON} FROM {SOURCE.TABLA} WHERE {SOURCE.ID} = @{SOURCE.ID}", new { id });
		}
		...
		{
			using var connection = new SqlConnection(_connectionString);
			var id = await connection.QuerySingleAsync<int>($@"INSERT INTO {SOURCE.TABLA} ({SOURCE.NAME}, {SOURCE.ICON}) VALUES (@{SOURCE.NAME}, @{SOURCE.ICON}) 
        ...
        {
            using var connection = new SqlConnection(_connectionString);
            await connection.ExecuteAsync($@"UPDATE {SOURCE.TABLA} SET {SOURCE.NAME} = @{SOURCE.NAME}, {SOURCE.ICON} = @{SOURCE.ICON} WHERE {SOURCE.ID} = @{SOURCE.ID}", source);
        }
